...
impl Compiler {
    ...
    fn program(&mut self) -> i32 {
        ...
        self.codes_index += 1;
        // 读取token
        self.fscanf_token(&mut self.token, &mut self.token1).unwrap();
...
// 自定义一个方法来判断文件是否已结束
    ...
    //     } else if self.token == "for" {
    //         es = self.for_stat(&mut child_node); // <for 语句>
    //     } else if self.token == "read" {
    ...
    //     } else if self.token == "call" {
    //         es = self.call_stat(&mut child_node); // <函数调用语句>
    //     } else if self.token == "ID" || self.token == "NUM" || self.token == "(" {
    ...
    //     es = self.lookup(&self.token1, &mut symbol_pos, self.last_defined_function);
    //     if es > 0 {
    //         return es;
    //     }
    ...
    // // 输出语法树
    // fn output_tree(&self, root: &Node, n: i32) {
    //     self.shift(n);
    //     println!("{}", root.s);
    ...
    //             self.output_tree(&root.child[i], n + 1); // 递归输出子树
    //         }
    //     }
    // }
